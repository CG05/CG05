...
const { Enemy, enemies } = require("./public/js/enemy.js");
const { Stats } = require("./public/js/stats.js");
const { Job, jobs } = require("./public/js/jobs.js");
const { Node, loadRoute, initRoutes, displayMap } = require("./public/js/map.js");
const { Item, Items, BossItems } = require("./public/js/item.js");
const { Equips } = require("./public/js/equips.js");
...
const db = require("./public/js/databaseIO.js");
const { type } = require("os");
const Database = db.Database;

app.set("views", path.join(__dirname, '/views')); //path 설정
...
async function setDB() {
  const { authParsed, userParsed } = await db.loadDB();
  console.log(authParsed);

  authDB = new Database(new Map(Object.entries(authParsed)));
  userDB = new Database(new Map(Object.entries(userParsed)));
}

async function setAuthDB() {
  const authParsed = await db.loadAuth();
  console.log(authParsed);
...
async function setUserDB() {
  const userParsed = await db.loadUser();
...
async function signUp(id, pw) {
  await setAuthDB();
  const created = await authDB.create(id, { pw: pw });
  return created;
}

async function saveAuth() {
  console.log(authDB);
  await db.saveAuth(await authDB.read());
}
...
async function initPlayerData(name) {
  const initStats = new Stats({maxHp: 80, currentHp:80, attackPoint:10, deffense:0, deffensePierce:0, damageAmplify:1, criticalChance:0.01, criticalDamage:1.5});
  const listJobs = await jobs;
  const job = listJobs[0];
  const initialJob = new Job(
    job.name,
    job.comment,
    job.line,
    job.skills,
  ...
  const createUser = new Promise(async (res, rej) => {
    await setUserDB();
    const created = await userDB.create(name, {
      info: playerInfo,
      map: mapData,
      enemy: enemyInfo,
      state: stateData,
  ...
  createUser
    ...
    .then(async () => {
      await db.saveUser(await userDB.read()).then(() => {
...
async function saveData(name) {
  const savingPlayerData = new Promise(async (res, rej) => {
    const saved = await userDB.update(name, {
      info: await getPlayerInfo(name),
      map: await getPlayerMap(name),
      enemy: await getPlayerEnemy(name),
      state: await getPlayerState(name),
    });
    console.log(`Player name ${name}: player saved - ${saved}`);
    res();
  });
  return savingPlayerData.then(async () => {
    await db.saveUser(await userDB.read());
  });
}

async function loadData(name) {
  const loadingPlayerData = await userDB.read(name);
  const isLoadedData = loadingPlayerData.hasKey;
  const data = loadingPlayerData.value;
  if (isLoadedData) {
    await setPlayerOnline(name, {
...
async function emitRespond(name, res){
  const playerInfo = await getPlayerInfo(name);
  const enemy = await getPlayerEnemy(name);
  const nextFloor = playerInfo.currentPosition.floor;
  let isboss = playerInfo.currentPosition.stage === 13;
  io.emit("next respond", {msg: res.msg, type: res.type, isboss: isboss, nextFloor: nextFloor});
}
...
async function sendChoices(name) {
  ...
  switch (playerRoute) {
    ...
    case "B":
      ...
      if (playerStage < 12) {
        BAisBranched =
          routeA[playerStage + 1].branch && routeB[playerStage].branch;
        BCisBranched =
          routeB[playerStage].branch && routeC[playerStage + 1].branch;
...
async function moveOnSequence(name) {
  ...
  await emitRespond(name, { msg: _msg, type: "moveOn" });
  let i = 1;
...
async function sendLullRespond(name) {
  ...
  if (playerInfo.needMoveOn) {
    ...
    await emitRespond(name, { msg: _msg, type: "lull" });
    emitText(
      "text",
      "=================================================================================="
    );
...
async function processCommand(name, command) {
  ...
  switch (command) {
    ...
    case "/skills": //name,	level, effect, increasePerLevel, damageFactor, attackTimes, cooltime
      ...
      await emitRespond(name, { msg: undefined, type: "skills" });
      break;
    case "/items":
      i = 0;
      emitText(
      ...
      await emitRespond(name, { msg: undefined, type: "items" });
      break;
    case "/equips":
      i = 0;
      emitText(
      ...
      await emitRespond(name, { msg: undefined, type: "equips" });
      break;
    case "/shop":
      ...
      await emitRespond(name, { msg: undefined, type: "shop" });
      break;
    case "/reset":
...
async function renderShop(name) {
  ...
  if (playerInfo.shopCount === 0) {
    await emitRespond(name, {
      msg: `더이상 상점을 이용할 수 없습니다.`,
      type: "lull",
    });
...
async function processChoice(name, choice) {
  ...
  if (choice.includes("F")) {
    ...
    await emitRespond(name, { msg: _msg, type: type });
  } else {
    const currentRoute = choice;
    const currentStage = playerPosition.stage + 1;
...
async function playerAttack(name, controll) {
  ...
  if (controll === "기본공격") {
    ...
    for (let t = 0; t < playerControll.attackTimes; t++) {
      ...
      const skillResult = await playerData.skillAttackDamage(
        playerControll,
        enemyInfo
      );
  ...
  emitText(
    "player attack",
    `효과는 굉장했다! <${enemyInfo.name}>에게 총 [${damage.toFixed(
      1
    )}]의 데미지를 주었다.`
  );
  emitText(
...
async function processCombat(name, controll) {
  ...
  if (isFinished) {
    if (
      playerInfo.currentPosition.floor === 5 &&
      playerInfo.currentPosition.stage === 13
    ) {
      return endingSequence(name);
    }
    emitText(
      "text",
      `방금의 일격으로 <${enemyInfo.name}>이(가) 사망했다. 보상이 들어온다!`
    );
    ...
    if (rewardItem === null || rewardItem === undefined) {
      io.emit(
        "result",
        `경험치 [${rewardExp}]와 [${rewardGold}Gold]를 얻었다!`
      );
    } else {
      io.emit(
        "result",
    ...
    if (isLevelUp) {
      ...
      emitText(
        "text",
        `##레벨업!! 축하합니다. 레벨${info.level}이 되셨습니다.`
      );
    ...
    if (isDead) {
      if (type === "defaultAttack") {
        emitText(
          "enemy attack",
          `당신은 <${enemyInfo.name}>에게 [${damage.toFixed(
            1
          )}]의 데미지를 입고 사망하였습니다..`
        );
...
async function sendEnemyStats(name) {
  ...
  emitText("text", _msg);
  _msg = `hp: ${enemyInfo.stats.currentHp.toFixed(
    1
  )}/${enemyInfo.stats.maxHp.toFixed(1)}`;
  emitText("text", _msg);
  emitText(
    "text",
    "----------------------------------------------------------------------------------"
...
async function processEventChoice(name, choice) {
  ...
  switch (choice) {
    case "체력 20% 회복":
      ...
      break;
    case "공격력 증가":
      deltaStats = new Stats({maxHp:0, currentHp:0, attackPoint:playerFloor * 3, deffense:0, deffensePierce:0, damageAmplify:0, criticalChance:0, criticalDamage:0});
      ...
      break;
    case "방어력 증가":
      deltaStats = new Stats({maxHp:0, currentHp:0, attackPoint:playerFloor * 2, deffense:0, deffensePierce:0, damageAmplify:0, criticalChance:0, criticalDamage:0});
...
async function switchState(name) {
  ...
  switch (state) {
    ...
    case "event":
    case "rest":
      await emitRespond(name, {msg: undefined, type: state})
      break;
    default:
...
async function endingSequence(name) {
  emitText(
    "text",
    `세계의 의지를 무너뜨리고 당신은 승리하였습니다. 축하합니다!`
  );
...
io.on("connection", async (socket) => {
  ...
  socket.on("login", async (user) => {
    await setDB();
    let resId = "";
    let authInfo = await authDB.read(user.id);
    const auth = authInfo.value.pw === user.pw;
  ...
  socket.on("signUp", async (req) => {
    ...
    var { id, pw } = req;
    let unsigned = await signUp(id, pw);
    if (unsigned) {
      saveAuth();
    }
  ...
  socket.on("start", async (name) => {
    ...
    await emitRespond(name, {msg: undefined, type: undefined});
    await switchState(name);
  });
  ...
  socket.on("choice", async (res) => {
    ...
    await processChoice(name, choice);
    await saveData(name);
  });
